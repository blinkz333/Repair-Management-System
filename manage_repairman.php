...
$sql = $conn->query("delete from repair_man where Rep_ID = '$_REQUEST[id]'")or die (mysqli_error());
...
if($_REQUEST['admin']=='status'){
    ...
    $sql = $conn->query("update repair_man set Rep_Status = '$_REQUEST[modalStatus]'  where Rep_ID = '$_REQUEST[id]'")or die (mysqli_error());
...
?>
                ...
                <!--End Page Header -->
                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            <i class="fa fa-bar-chart-o fa-fw"></i> ตารางแสดงข้อมูลช่างซ่อม
	...
	<input name="txt_search" type="text" id="course" size="50" class="form-control" placeholder="ค้นหาข้อมูลกรุณากรอกชื่อช่าง" required="required" /><span class="input-group-btn">
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-12">
									...
									<div class="panel"><a href="manage_repairman_add.php"><input name="" type="button" class="btn btn-success" value="เพิ่มข้อมูลช่าง"></a></div>

                                        <table class="table table-striped table-bordered table-hover" id="table_example2">
                                            <thead>
                                                <tr>
                                                    <th width="5%"><div align="center">ลำดับที่</div></th>
                                                    <th width="auto"><div align="center">ชื่อช่าง</div></th>
...
$sql = $conn->query("select * from repair_man order by Rep_ID DESC limit $start,$perpage")or die (mysqli_error());

//หาจำนวน row ทั้งหมด ของ ข้อมูลที่ถูกแสดงเพื่อจะเอาไปทำการแบ่งหน้า
$sql2 = $conn->query("select * from repair_man order by Rep_ID DESC")or die (mysqli_error());
...
$sql = $conn->query("select * from repair_man  where Rep_Name like '%$_REQUEST[txt_search]%' order by Rep_ID desc limit $start,$perpage")or die (mysqli_error());

//หาจำนวน row ทั้งหมด ของ ข้อมูลที่ถูกค้นหาเพื่อจะเอาไปทำการแบ่งหน้า
$sql2 = $conn->query("select * from  repair_man  where Rep_Name like '%$_REQUEST[txt_search]%' order by Rep_ID desc")or die (mysqli_error());
$total_record = $sql2->num_rows;
$total_page = ceil($total_record / $perpage);
		}
  ...
  while ($show= $sql->fetch_assoc()) {
     ...
     if($show['Rep_Status']==0){$status =  '<span class=text-info>ว่างงาน</span>';}
else if($show['Rep_Status']==1){$status =  '<span class=text-danger>ไม่ว่างงาน</span>';}
?>
                  <tr>
                  <td><div align="center"><?php echo $i++;?></div></td>
                  <td><div align="center"><?php echo $show['Rep_Name'];?></div></td>
                  <td><div align="center"><?php echo $show['Rep_Address'];?></div></td>
                  <td><div align="center"><?php echo $show['Rep_Tel'];?></div></td>
                  <td><div align="center"><?php echo $status; ?></div></td>
                  <td><div align="center">
                  <a href="#order<?php echo $show['Rep_ID'];?>" data-toggle="modal"><input name="" type="button" class="btn btn-warning" value="จัดการสถานะช่าง"></a>&nbsp;
                  <a href="manage_repairman_case.php?id=<?php echo $show['Rep_ID'];?>" data-toggle="modal"><input name="" type="button" class="btn btn-success" value="ดูงานที่รับผิดชอบ"></a>&nbsp;
                  <a href="manage_repairman_edit.php?id=<?php echo $show['Rep_ID'];?>"><input name="" type="button" class="btn btn-primary" value="แก้ไขข้อมูลช่าง"></a>&nbsp;
                  <a href="?admin=delete&id=<?php echo $show['Rep_ID'];?>"><input name="" type="button" class="btn btn-danger" value="ลบข้อมูลช่าง"></a>
                  </div></td>
                  </tr>
